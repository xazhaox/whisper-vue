...
import { contentTheme } from "@/styles/theme/content";
...
export const useTheme = () => {
  ...
  const changePrimary = (val: string | null) => {
    ...
    // 计算主题颜色变化
    document.documentElement.style.setProperty("--a-color-primary", val);
    document.documentElement.style.setProperty(
      "--a-color-primary-dark-2",
      isDark.value ? `${getLightColor(val, 0.2)}` : `${getDarkColor(val, 0.3)}`
    );
    for (let i = 1; i <= 9; i++) {
      const primaryColor = isDark.value ? `${getDarkColor(val, i / 10)}` : `${getLightColor(val, i / 10)}`;
      document.documentElement.style.setProperty(`--a-color-primary-light-${i}`, primaryColor);
    }
